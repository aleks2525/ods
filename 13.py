...
CANDIDATES_PER_ITEM = 1200 # Увеличено для лучшего покрытия
USERS_PER_ITEM = 100
MAX_ASSIGN_PER_USER = 100
EMB_DIM = 128 # Увеличено
SIM_TOPK = 500
ALS_FACTORS = 192 # Увеличено
ALS_ITERATIONS = 25
BPR_FACTORS = 128
BPR_ITERATIONS = 20

# Оптимизированные веса (критично!)
DEFAULT_WEIGHTS = {
"W_ALS": 12.0,
"W_BPR": 8.0, # Новая модель
"W_SIMHITS": 6.0,
"W_AUTHOR": 4.0,
"W_POPULARITY": 0.2,
"W_TIME_DECAY": 2.0,
...
"W_RECENCY": 2.5,
"W_DIVERSITY": 1.0,
"W_USER_ITEM_SIM": 3.0, # Новая фича
}

EXPLICIT_ACTIONS = ["like", "share", "bookmark", "click_on_author"]
ACTION_WEIGHTS = {
"like": 1.0,
"share": 3.0, # share важнее
"bookmark": 2.0,
"click_on_author": 1.5,
}
...
index.add(item_vecs)
index.nprobe = 64 # Увеличено для лучшего качества
...
ibcf_sc += sim
ibcf_weighted += sim * sim # Квадратичный вес
n_sim_hits += 1
...
# Активность пользователя в последнее время
...
# =========================
# Compute user embeddings from ALS
# =========================
...
# Собираем эмбеддинги items пользователя
embeddings = []
for iid in items:
...
# Считаем среднее попарное расстояние
...
# Среднее сходство (без диагонали)
n = len(embeddings)
...
# Используем все доступные недели (важно!)
...
# Build indices с взвешиванием действий
...
# Train BPR (дополнительная модель)
...
# Для BPR используем бинарную матрицу
...
# Только если есть явное взаимодействие
...
# Candidates (улучшенная стратегия)
...
# Train XGBoost (оптимальные параметры для качества)
...
'learning_rate': 0.03, # Меньше для точности
'subsample': 0.85,
...
model = xgb.train(params, dtrain, num_boost_round=300) # Больше итераций
info("✅ XGBoost обучен.")
