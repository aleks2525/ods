...
# -*- coding: utf-8 -*-
# =========================
# 🎯 Версия для достижения 0.35+ метрики (оптимизированная загрузка)
# =========================
import os
...
def find_local_weeks(subsample_name: str) -> List[str]:
    """Находит уже загруженные недели локально"""
    train_dir = os.path.join(DATA_ROOT, "subsamples", subsample_name, "train")
    ...
    for f in os.listdir(train_dir):
        if f.endswith('.parquet'):
            weeks.append(f.replace('.parquet', ''))
...
def ensure_download(filename: str) -> str:
    ...
    try:
        ...
        warn(f"Ошибка загрузки {filename}: {e}")
    return local_path
...
def ensure_dataset_files(subsample_name: str, weeks: Optional[List[str]], val_week: Optional[str] = None):
    ...
    local_weeks = find_local_weeks(subsample_name)

    if weeks is None:
        if local_weeks:
            info(f"Используем локальные недели: {local_weeks}")
            weeks = local_weeks
        else:
            info("Локальных недель не найдено, загружаем из HF...")
    ...
    path_items_meta = os.path.join(DATA_ROOT, "metadata", "items_metadata.parquet")
    if not os.path.exists(path_items_meta):
        path_items_meta = ensure_download("metadata/items_metadata.parquet")

    path_item_embs = os.path.join(DATA_ROOT, "metadata", "item_embeddings.npz")
    if not os.path.exists(path_item_embs):
        path_item_embs = ensure_download("metadata/item_embeddings.npz")

    # Example файл
    example_path = None
    for fname in ["example.parquet", "subsamples/example.parquet"]:
        test_path = os.path.join(DATA_ROOT, fname)
    ...
    if example_path is None:
        try:
            ...
            if example_candidates:
                example_path = ensure_download(example_candidates[0])
        except:
            warn("Не удалось найти example.parquet")

    # Validation файл
    val_path = None
    if val_week:
        val_path = os.path.join(DATA_ROOT, "subsamples", subsample_name, "validation", f"{val_week}.parquet")
    ...
    return weeks, path_items_meta, path_item_embs, example_path, val_path
...
def run_pipeline(subsample_name: str, weeks: Optional[List[str]], output_path: str, val_week: str = "week_25"):
    weeks, path_items_meta, path_item_embs, example_path, val_path = ensure_dataset_files(subsample_name, weeks, val_week)
    ...
    # Используем последние 7 недель (оптимально для скорости и качества)
    ...
    for week_idx, w in enumerate(tqdm(recent_weeks, desc="Загрузка недель")):
        p = os.path.join(DATA_ROOT, "subsamples", subsample_name, "train", f"{w}.parquet")

        if not os.path.exists(p):
            warn(f"Файл не найден: {p}, пропускаем...")
            continue

        # Читаем только нужные колонки
        try:
            df_week = pl.read_parquet(p, columns=["user_id", "item_id"] + EXPLICIT_ACTIONS)
        except Exception as e:
            warn(f"Ошибка чтения {w}: {e}")
            continue
    ...
    info(f"✅ Индексы: {len(user_to_items)} users, {len(item_to_users)} items")
    ...
    for idx, w in enumerate(recent_weeks):
        ...
        df_week = pl.read_parquet(p, columns=["user_id", "item_id"])
    ...
    if not example_path or not os.path.exists(example_path):
        raise RuntimeError("Example файл не найден!")
    ...
    if val_path and os.path.exists(val_path):
        ...
        try:
            df_val = pl.read_parquet(val_path, columns=["user_id", "item_id"] + EXPLICIT_ACTIONS)
    ...
    for author_id, lst in author_to_sorted_fans.items():
        ...
        author_to_sorted_fans[author_id] = lst[:1000]
    ...
    for iid in tqdm(example_items, desc="Кандидаты"):
        ...
        for si, sim in similar_cache.get(iid, [])[:100]:
            if si in item_to_users:
                users = item_to_users[si]
                sample_size = min(len(users), int(150 * sim))
                cand_users |= set(list(users)[:sample_size])

        author = item_to_author.get(iid)
        if author is not None and author in author_to_sorted_fans:
            for u, _ in author_to_sorted_fans[author][:200]:
                cand_users.add(u)

        if iid in cooc_map:
            for co_iid, cnt in sorted(cooc_map[iid].items(), key=lambda x: -x[1])[:80]:
                if co_iid in item_to_users:
                    users = item_to_users[co_iid]
                    sample_size = min(len(users), int(80 * math.log1p(cnt)))
                    cand_users |= set(list(users)[:sample_size])
